...
class SelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        ...
        if self.is_cross_attention:
            self.c_attn = Conv1D(2 * self.n_embd, self.n_embd)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            assert hasattr(self, "q_attn"), "If class is used as cross attention, the weights `q_attn` have to be defined."
...
class Block(nn.Module):
    def __init__(self, config, scale=False):
        ...
        if config.add_cross_attention:
            self.crossattention = SelfAttention(config, is_cross_attention=True)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            assert hasattr(self, "crossattention"), "If `encoder_hidden_states` are passed, cross-attention layers must be instantiated."
